...
const Next: React.FC = () => {
  const [structure, setStructure] = useState<Node | null>(null);
    ...
    const [projectName, setProjectName] = useState<string>('my-next-app');

    const fetchStructure = async () => {
        fetchStructureProject('http://localhost:3003/next-app/fetch-structure', {
            appName: projectName,
        }, setStructure)
    };

    const handleSaveStructure = async () => {
        handleSaveStructureProject('http://localhost:3003/next-app/create', {
            structure: structure,
